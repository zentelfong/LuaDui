...
extern luv_handle_t* luv_setup_handle(lua_State* L);
...
extern void luv_check_callback(lua_State* L, luv_handle_t* data, luv_callback_id id, int index);

// Lookup a function and call it with nargs
// If there is no such function, pop the args.
extern void luv_call_callback(lua_State* L, luv_handle_t* data, luv_callback_id id, int nargs);

// Push a userdata on the stack from a handle
extern void luv_find_handle(lua_State* L, luv_handle_t* data);

// Recursivly free the luv_handle and all event handlers
extern void luv_cleanup_handle(lua_State* L, luv_handle_t* data);
